...
(defn- tokenize-args
  ...
  (let [{:keys [subcommand]} (apply hash-map options)]
    (loop [opts [] argv [] remaining-args (seq args)]
      (if-let [car (first remaining-args)]
        (let [cdr (rest remaining-args)]
          (cond
          ;; Double dash always ends options processing
          (re/search #"^--$" car) (recur opts (into argv cdr) [])
          ;; Long options with assignment always passes optarg, required or not
          (re/search #"^--\S+=" car) (recur (conj opts (into [:long-opt] (s/split car #"=" 2)))
                                        argv cdr)
          ;; Long options, consumes cdr head if needed
          (re/search #"^--" car) (let [[optarg remaining] (if (contains? required-set car)
                                                          [(first cdr) (rest cdr)]
                                                          [nil cdr])]
                                 (recur (conj opts (into [:long-opt car] (if optarg [optarg] [])))
                                        argv remaining))
          ;; Short options, expands clumped opts until an optarg is required
          (re/search #"^-." car) (let [[os remaining] (loop [os [] [c & cs] (rest car)]
                                                      (let [o (str \- c)]
      ...
      (when (seq unknown-keys)
        (let [msg (str "Warning: The following options to parse-opts are unrecognized: "
          ...
          #?(:clj  (binding [*out* *err*] (println msg))
             ...
             :lpy  (binding [*out* *err*] (println msg)))))))
...
(defn- validate [value spec opt optarg]
  (let [{:keys [validate-fn validate-msg]} spec]
    (or (loop [[vfn & vfns] validate-fn [msg & msgs] validate-msg]
          (when vfn
            (if (try (vfn value) (catch #?(:clj Throwable :cljr Exception :cljs :default :lpy python/Exception) _))
              (recur vfns msgs)
...
(defn- parse-value [value spec opt optarg]
  (let [{:keys [parse-fn]} spec
        [value error] (if parse-fn
                        (try
                          [(parse-fn value) nil]
                          (catch #?(:clj Throwable :cljr Exception :cljs :default :lpy python/Exception) e
...
(defn format-lines
  ...
  (let [fmt (make-format lens)]
    (map #(s/trimr (apply basilisp.core/format fmt %)) parts)))
  ...
  (let [docs (into (map build-doc specs)
    ...
    (doseq [v vs]
      (let [fmt (make-format (take-nth 2 v))]
        (print (apply basilisp.core/format fmt (take-nth 2 (rest v)))))
      (prn))))
      ...
      (let [opt  (first args)
        ...
        (cond
          ...
          (throw #?(:clj  (Exception. (str "'" opt "' is not a valid argument"))
                    ...
                    :lpy (python/Exception (str "'" opt "' is not a valid argument"))))
...
(defn- switches-for
  ...
  (mapcat identity
          (for [switch switches]
            (cond (and flag (flag? switch))
                  [(s/replace switch #"\[no-\]" "no-") (s/replace switch #"\[no-\]" "")]

                  (and flag (s/starts-with? switch "--"))
                  [(s/replace switch #"--" "--no-") switch]
