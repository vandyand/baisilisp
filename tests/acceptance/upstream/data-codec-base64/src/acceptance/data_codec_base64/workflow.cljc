(ns acceptance.data-codec-base64.workflow
...
(defn public-summary []
  (sort (map name (keys (ns-publics 'clojure.data.codec.base64)))))
...
(defn decoded-or-error [s]
  (let [bs (ascii-bytes s)]
    (try
      {:error? false
       :pad-error? (error? #(b64/pad-length bs 0 (count bs)))
       :pad (when-not (error? #(b64/pad-length bs 0 (count bs)))
...
(defn decoder-boundary-summary []
  {:permissive (into {}
                     (map (fn [s] [s (decoded-or-error s)]))
                     ["" "A" "AA" "AAA" "AAAA" "AAAAA" "abc" "abcd"
                      "!!!!" "Zg=a" "=" "==" "===" "====" "A=" "A=="
                      "A===" "AA=" "AA==" "AAA=" "AAA=="])
   :table {:low-invalid (decoded-bytes-or-error [35 35 35 35])
...
(defn generated-case [seed]
  (let [s1 (next-seed seed)
        ...
        suffix (seeded-bytes s3 (mod s3 9))
        ...
        enc-out (byte-array (b64/enc-length (count payload)))
        dec-out (byte-array (count payload))
        enc-written (b64/encode! source (count prefix) (count payload) enc-out)
        dec-written (b64/decode! enc-out 0 enc-written dec-out)]
    {:seed seed
     :size (count payload)
     :encoded-length (count encoded)
     :encoded-into-length enc-written
     :decode-written dec-written
     :roundtrip? (= payload (unsigned-bytes decoded))
     :encode-into? (= (ascii encoded) (ascii enc-out))
     :decode-into? (= payload (unsigned-bytes dec-out))
     :next-seed s3}))
